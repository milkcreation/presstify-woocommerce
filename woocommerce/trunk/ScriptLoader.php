...
{    
     ...
     */
    protected static $WcEnqueueStyles       = array(
     ...
     * @see wp-content/plugins/woocommerce/includes/class-wc-frontend-scripts.php
     */
    protected static $WcEnqueueScripts      = array(
    ...
    {        
        ...
        foreach( Tags::getAll() as $tag ) :            
            if( file_exists( get_stylesheet_directory(). "/css/wc-{$tag}". static::$Min .".css" ) ) :
                tify_register_style(
                    'tify_wc_'. $tag,
                    array(
                        'src'        => get_stylesheet_directory_uri(). "/css/wc-{$tag}". static::$Min .".css"
                    )
                );
        ...
        // SCRIPTS       
        foreach( Tags::getAll() as $tag ) :            
            if( file_exists( get_stylesheet_directory(). "/js/wc-{$tag}". static::$Min .".js" ) ) :
                tify_register_script(
                    'tify_wc_'. $tag,
                    array(
                        'src'        => get_stylesheet_directory_uri(). "/js/wc-{$tag}". static::$Min .".js"
                    )
                );
    ...
    final public function wp_enqueue_scripts()
    {
        ...
        if( file_exists( get_stylesheet_directory(). "/css/wc-global". static::$Min .".css" ) ) :
            wp_enqueue_style( 'tify_wc_global', get_stylesheet_directory_uri(). "/css/wc-global". static::$Min .".css" );
        endif;
        if( file_exists( get_stylesheet_directory(). "/js/wc-global". static::$Min .".js" ) ) :
            wp_enqueue_script( 'tify_wc_global', get_stylesheet_directory_uri(). "/js/wc-global". static::$Min .".js" );
        endif;
        ...
        foreach( Tags::getAll() as $tag ) :
            if( Tags::isCurrent( $tag ) ) :
                ...
                if( is_callable( array( $this, 'enqueue_scripts_before_'. $tag ) ) ) :
                    call_user_func( array( $this, 'enqueue_scripts_before_'. $tag ) );
                endif;
                ...
                if( is_callable( array( $this, 'enqueue_scripts_after_'. $tag ) ) ) :
                    call_user_func( array( $this, 'enqueue_scripts_after_'. $tag ) );
                endif;
            endif;
        endforeach;
    }
